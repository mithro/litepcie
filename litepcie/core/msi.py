...
class LitePCIeMSI(LiteXModule):
    def __init__(self, width=32):
        ...
        self.enable = CSRStorage(
            width,
            description="""MSI Enable Control.\n
           Write bit(s) to ``1`` to enable corresponding MSI IRQ(s).""",
        )
        self.clear = CSRStorage(
            width,
            description="""MSI Clear Control.\n
           Write bit(s) to ``1`` to clear corresponding MSI IRQ(s).""",
        )
        self.vector = CSRStatus(
            width,
            description="""MSI Vector Status.\n
        ...
        self.sync += [
            ...
            If(self.source.ready, msi.eq(self.irqs & enable)),
        ]
        ...
        for i in reversed(range(width)):  # Priority given to lower indexes.
            self.comb += [
                If(
                    ...
                    If(self.source.ready, clear.eq(1 << i)),
                )
            ]
...
class LitePCIeMSIX(LiteXModule):
    def __init__(self, endpoint, width=32, default_enable=0):
        ...
        self.enable = CSRStorage(
            width,
            description="""MSI-X Enable Control.\n
        ...
        self.specials.table = Memory(
            4 * 32, width, init=[0b1 for _ in range(width)]
        )  # MSI-X Table / Masked by default.
        ...
        fsm.act(
            ...
            table_port.re.eq(1),
            If(msix_valid, NextValue(msix_clear_on_ready, msix_clear), NextState("ISSUE-WRITE")),
        )
        self.comb += [
        ...
        fsm.act(
            ...
            port.source.we.eq(1),
            If(port.source.ready | msix_mask, clear.eq(msix_clear_on_ready), NextState("IDLE")),
        )
