...
class EndiannessSwap(LiteXModule):
    ...
    def __init__(self, sink, source):
        ...
        n = len(sink.ctrl)
        for i in range(n):
            self.comb += source.data[8 * i : 8 * (i + 1)].eq(
                sink.data[8 * (n - 1 - i) : 8 * (n - 1 - i + 1)]
            )
...
class Scrambler(Module):
    ...
    def __init__(self, reset=0x7DBD):
        ...
        for i in range(4):
            self.comb += [
                If(
                    ~self.enable | sink.ctrl[i],  # K codes shall not be scrambled.
                    source.data[8 * i : 8 * (i + 1)].eq(sink.data[8 * i : 8 * (i + 1)]),
                ).Else(
                    source.data[8 * i : 8 * (i + 1)].eq(
                        sink.data[8 * i : 8 * (i + 1)] ^ unit.value[8 * i : 8 * (i + 1)]
                    )
                )
            ]
...
class RawDescrambler(Module):
    ...
    def __init__(self, reset=0xFFFF):
        ...
        # Descramble data
        self.comb += [sink.connect(scrambler.sink), scrambler.source.connect(source)]
...
class RawWordAligner(LiteXModule):
    ...
    def __init__(self):
        ...
        for i in reversed(range(4)):
            self.comb += [
                If(
                    ...
                    If(
                        sink.ctrl[i] & (sink.data[8 * i : 8 * (i + 1)] == COM.value),
                        update.eq(1),
                        alignment.eq(i),
                    ),
                )
        ...
        self.sync += [
            If(sink.valid & sink.ready, If(self.enable & update, alignment_d.eq(alignment)))
        ]
...
class RawDatapath(LiteXModule):
    ...
    def __init__(self, clock_domain="sys", phy_dw=16):
        ...
        converter = stream.StrideConverter(
            [("data", phy_dw), ("ctrl", phy_dw // 8)], [("data", 32), ("ctrl", 4)], reverse=False
        )
    ...
    def __init__(self):
        ...
        fsm.act(
            ...
            If(
                sink.valid,
                If(
                    sink.ctrl[0] & (sink.data[0 * 8 : 1 * 8] == 0xFB),
                    NextValue(alignment, 0b00),
                    NextState("RECEIVE-0"),
                ),
                If(
                    sink.ctrl[1] & (sink.data[1 * 8 : 2 * 8] == 0xFB),
                    NextValue(alignment, 0b01),
                    NextState("RECEIVE-0"),
                ),
                If(
                    sink.ctrl[2] & (sink.data[2 * 8 : 3 * 8] == 0xFB),
                    NextValue(alignment, 0b10),
                    NextState("RECEIVE-0"),
                ),
                If(
                    sink.ctrl[3] & (sink.data[3 * 8 : 4 * 8] == 0xFB),
                    NextValue(alignment, 0b11),
                    NextState("RECEIVE-0"),
                ),
            ),
        )
        fsm.act("RECEIVE-0", If(sink.valid, NextValue(first, 1), NextState("RECEIVE-1")))
        fsm.act(
            "RECEIVE-1",
            If(
                sink.valid,
            ...
            If(
                sink.valid & ~first,
                If(
                    sink_ctrl_dd[0] & (sink_data_dd[0 * 8 : 1 * 8] == 0xFD),
                    source.last.eq(1),
                    NextState("IDLE"),
                ),
                If(
                    sink_ctrl_dd[1] & (sink_data_dd[1 * 8 : 2 * 8] == 0xFD),
                    source.last.eq(1),
                    NextState("IDLE"),
                ),
                If(
                    sink_ctrl_dd[2] & (sink_data_dd[2 * 8 : 3 * 8] == 0xFD),
                    source.last.eq(1),
                    NextState("IDLE"),
                ),
                If(
                    sink_ctrl_dd[3] & (sink_data_dd[3 * 8 : 4 * 8] == 0xFD),
                    source.last.eq(1),
                    NextState("IDLE"),
                ),
            ),
...
class TLPFilterFormater(LiteXModule):
    def __init__(self):
        ...
        self.conv = conv = stream.StrideConverter(
            description_from=phy_layout(32), description_to=phy_layout(64), reverse=False
        )
        self.comb += [
        ...
        fsm.act(
            ...
            If(
                ...
                # PTM Request.
                If(
                    sink.data[24:32] == fmt_type_dict["ptm_req"],
                ...
                .Elif(
                    sink.data[24:32] == fmt_type_dict["ptm_res"],
                    fifo.sink.valid.eq(1),
                    fifo.sink.dat.eq(sink.data),
        ...
        fsm.act(
            ...
            If(
                ...
                If(count == 0, fifo.sink.last.eq(1), NextState("END")),
                If(sink.last, fifo.sink.last.eq(1), NextState("IDLE")),
            ),
        )
        fsm.act("END", If(sink.valid & sink.last, NextState("IDLE")))
...
class PCIePTMSniffer(LiteXModule):
    def __init__(self, rx_rst_n, rx_clk, rx_data, rx_ctrl):
        self.source = source = stream.Endpoint(
            [("message_code", 8), ("master_time", 64), ("link_delay", 32)]
        )
        ...
        self.tlp_depacketizer = ClockDomainsRenamer("sniffer")(
        ...
        self.comb += [
            self.tlp_depacketizer.ptm_source.connect(
                cdc.sink, keep={"valid", "ready", "master_time"}
            ),
