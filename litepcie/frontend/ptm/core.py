...
class PTMCapabilities(LiteXModule):
    def __init__(
        ...
        ptm_capability_init = {
            PTM_HEADER_REG: (
                (1 << PTM_HEADER_VERSION_OFFSET) * 0x01 | (1 << PTM_HEADER_ID_OFFSET) * 0x1F
            ),
            PTM_CAPABILITY_REG: (
                (1 << PTM_CAPABILITY_REQUESTER_CAPABLE_OFFSET) * requester_capable
        ...
        mem = Memory(
            32,
            PTM_STRUCTURE_REGS,
            init=[ptm_capability_init[4 * i] for i in range(PTM_STRUCTURE_REGS)],
        )
        ...
        fsm.act(
            "IDLE",
            If(conf_ep.valid, If(conf_ep.we, NextState("WRITE-MEM")).Else(NextState("READ-MEM"))),
        )
        ...
        fsm.act(
            ...
            comp_ep.dat.eq(dat),
            If(comp_ep.valid & comp_ep.ready, conf_ep.ready.eq(1), NextState("IDLE")),
        )
        ...
        self.comb += [
            ...
            self.ptm_effective_granularity.eq(
                (mem_ctrl_port.dat_r >> PTM_CONTROL_EFFECTIVE_GRANULARITY_OFFSET) & 0b1111_1111
            ),
        ]
...
class PTMRequester(LiteXModule):
    def __init__(self, pcie_endpoint, pcie_ptm_sniffer, sys_clk_freq, with_csr=True):
        ...
        time_cdc = stream.ClockDomainCrossing(
            [("time", 64)],
        ...
        time = Signal(64)
        self.sync += If(time_cdc.source.valid, time.eq(time_cdc.source.time))
        ...
        fsm.act("START", If(self.enable, NextState("INVALID-PTM-CONTEXT")))
        fsm.act("INVALID-PTM-CONTEXT", If(self.trigger, NextState("ISSUE-PTM-REQUEST")))
        fsm.act(
            "ISSUE-PTM-REQUEST",
            self.busy.eq(1),
            req_ep.valid.eq(1),
            ...
            If(req_ep.ready, NextValue(self.t1, time), NextState("WAIT-PTM-RESPONSE")),
        )
        ...
        fsm.act(
            ...
            self.busy.eq(1),
            If(
                pcie_ptm_sniffer.source.valid,
                If(
                    pcie_ptm_sniffer.source.message_code == PTM_RESPONSE_MESSAGE_CODE,
                    If(
                        pcie_ptm_sniffer.source.master_time
                        == 0,  # FIXME: Add Response/ResponseD indication.
                        NextState("WAIT-1-US"),
                    ).Else(
        ...
        fsm.act(
            ...
            If(self.req_timer.done, NextState("ISSUE-PTM-REQUEST")),
        )
        fsm.act(
            "VALID-PTM-CONTEXT",
            self.valid.eq(1),
            NextValue(self.update, 0),
            If(self.trigger, NextState("ISSUE-PTM-REQUEST")),
            If(self.invalidate, NextState("INVALID-PTM-CONTEXT")),
        )
    ...
    def add_csr(self, sys_clk_freq, default_enable=0, phy_tx_delay=40e-9, phy_rx_delay=100e-9):
        ...
        self._phy_tx_delay = CSRStatus(
            32, reset=int(phy_tx_delay * 1e9), description="PHY TX logic delay (in ns)."
        )
        self._phy_rx_delay = CSRStatus(
            32, reset=int(phy_rx_delay * 1e9), description="PHY RX logic delay (in ns)."
        )
