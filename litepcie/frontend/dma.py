...
class LitePCIeDMAScatterGather(LiteXModule):
    ...
    def __init__(self, depth, address_width=32):
        ...
        self.value = CSRStorage(
            ...
            fields=[
                CSRField(
                    "address_lsb",
                    size=32,
                    description="32-bit LSB Address of the descriptor (bytes-aligned).",
                ),
                CSRField(
                    "length", size=24, description="24-bit Length  of the descriptor (in bytes)."
                ),
                CSRField(
                    "irq_disable", size=1, description="IRQ Disable Control of the descriptor."
                ),
                CSRField(
                    "last_disable", size=1, description="Last Disable Control of the descriptor."
                ),
            ],
        ...
        self.we = CSRStorage(
            32,
            description="Write and 32-bit MSB Address of the descriptor (bytes-aligned)",
            fields=[
                CSRField(
                    "address_msb",
                    size=32,
                    description="32-bit MSB Address of the descriptor (bytes-aligned), in 64-bit mode.",
                ),
            ],
        )
        ...
        self.loop_status = CSRStatus(
            fields=[
                CSRField(
                    "index",
                    size=16,
                    description="Index of the last descriptor executed in the DMA descriptor table.",
                ),
                CSRField(
                    "count", size=16, description="Loops of the DMA descriptor table since started."
                ),
            ],
        ...
        self.sync += [
            ...
            If(
                ...
                table.source.valid & table.source.ready,
                # Update Loop Status with current Loop Index/Count.
                # Loop Mode.
                If(
                    loop_mode & table.source.first,
                    # Reset Index.
                    loop_index.eq(0),
                    ...
                    # Increment Count.
                    If(loop_index == (2**16 - 1), loop_count.eq(loop_count + 1)),
                ),
            )
        ]
...
class LitePCIeDMADescriptorSplitter(LiteXModule):
    ...
    def __init__(self, max_size, address_width):
        ...
        self.source = source = stream.Endpoint(
            descriptor_layout(address_width=address_width, with_user_id=True)
        )
        ...
        fsm.act(
            ...
            If(sink.length > max_size, NextValue(source.length, max_size)).Else(
                NextValue(source.last, 1), NextValue(source.length, sink.length)
            ),
            # Wait for a descriptor and go to RUN.
            If(sink.valid, NextState("RUN")),
        )
        self.comb += [
        ...
        fsm.act(
            ...
            If(
                ...
                If(length_next > max_size, NextValue(source.length, max_size)).Else(
                    NextValue(source.last, 1),
                    NextValue(source.length, length_next),
                ),
                # On last or terminate...
                If(
                    source.last | self.terminate,
                    # Accept Descriptor.
                    sink.ready.eq(1),
                    # Increment User-ID.
...
class LitePCIeDMAReader(LiteXModule):
    ...
    def __init__(
        self, endpoint, port, with_table=True, table_depth=256, address_width=32, data_width=None
    ):
        self.port = port
        ...
        self._enable = CSRStorage(
            size=2,
            description="DMA Reader Control. Write ``1`` to enable DMA Reader.",
            reset=0 if with_table else 1,
        )
        ...
        if with_table:
            ...
            self.desc_sink = stream.Endpoint(
                descriptor_layout(address_width=address_width)
            )  # Expose a Descriptor sink.
        ...
        splitter = LitePCIeDMADescriptorSplitter(
            max_size=endpoint.phy.max_request_size, address_width=address_width
        )
        ...
        self.sync += If(
            port.sink.valid & port.sink.first & port.sink.ready, last_user_id.eq(port.sink.user_id)
        )
        ...
        self.comb += [
            ...
            If(
                splitter.source.valid & splitter.source.ready,
                pending_words_queue.eq(splitter.source.length[length_shift:]),
            ),
            # Dequeue Pending words as Read Responses are received.
            If(data_fifo.source.valid & data_fifo.source.ready, pending_words_dequeue.eq(1)),
        ]
        ...
        fsm.act(
            ...
            If(
                ...
                splitter.source.valid & (pending_words < (data_fifo_depth - max_words_per_request)),
        ...
        # IRQ --------------------------------------------------------------------------------------
        self.comb += If(
            splitter.source.valid & splitter.source.ready & splitter.source.last,
...
class LitePCIeDMAWriter(LiteXModule):
    ...
    def __init__(
        ...
        self._enable = CSRStorage(
            size=2,
            description="DMA Writer Control. Write ``1`` to enable DMA Writer.",
            reset=0 if with_table else 1,
        )
        ...
        # DMA descriptors need to be splitted in descriptors of max_request_size (negotiated at link-up)
        splitter = LitePCIeDMADescriptorSplitter(
            max_size=endpoint.phy.max_payload_size, address_width=address_width
        )
        splitter = ResetInserter()(splitter)
        self.splitter = splitter
        if with_table:
        ...
        data_fifo = stream.SyncFIFO(
            [("data", endpoint.phy.data_width)], data_fifo_depth, buffered=True
        )
        self.data_fifo = ResetInserter()(data_fifo)
        ...
        fsm.act(
            ...
            If(
                ...
                splitter.source.valid & (data_fifo.level >= splitter.source.length[length_shift:]),
                NextState("MEM-WR"),
            ),
        )
        # Report Idle Status.
...
class LitePCIeDMALoopback(LiteXModule):
    ...
    def __init__(self, data_width):
        self.enable = CSRStorage(
            description="""DMA Loopback Enable Control.\n
        ...
        self.comb += [
            If(self.enable.storage, self.sink.connect(self.source)).Else(
                self.sink.connect(self.next_source), self.next_sink.connect(self.source)
            )
        ]
        ...
        self.sync += [
            ...
            .Else(
                ...
                If(
                    ...
                    If((self.enable.fields.mode == 0b01) & self.sink.valid, synced.eq(1)),
                    # Synchronization only on PPS. Reader and Writer are independent
                    If(self.enable.fields.mode == 0b10, synced.eq(1)),
                )
            )
        ]
        self.comb += [
...
class LitePCIeDMABuffering(LiteXModule):
    ...
    def __init__(
        self, data_width, with_writer, with_reader, writer_depth, reader_depth, dynamic_depth=True
    ):
        self.sink = stream.Endpoint(dma_layout(data_width))
        self.source = stream.Endpoint(dma_layout(data_width))
        ...
        if with_reader:
            ...
            self.reader_fifo_control = CSRStorage(
                fields=[
                    CSRField(
                        "depth",
                        offset=0,
                        size=24,
                        reset=reader_depth,
                        description=f"DMA Reader FIFO depth (in {data_width}-bit words).",
                    ),
            ...
            self.reader_fifo_status = CSRStatus(
                fields=[
                    CSRField(
                        "level",
                        offset=0,
                        size=24,
                        description=f"DMA Reader FIFO level (in {data_width}-bit words).",
                    )
                ]
            )
        ...
        if with_writer:
            ...
            self.writer_fifo_control = CSRStorage(
                fields=[
                    CSRField(
                        "depth",
                        offset=0,
                        size=24,
                        reset=writer_depth,
                        description=f"DMA Writer FIFO depth (in {data_width}-bit words).",
                    ),
                    CSRField("scratch", offset=24, size=4, description="Software Scratchpad."),
                    CSRField(
                        "level_mode",
            ...
            self.writer_fifo_status = CSRStatus(
                fields=[
                    CSRField(
                        "level",
                        offset=0,
                        size=24,
                        description=f"DMA Writer FIFO level (in {data_width}-bit words).",
                    )
                ]
            )
        ...
        # Reader FIFO.
        if with_reader:
            reader_fifo = ResetInserter()(
                SyncFIFO(dma_layout(data_width), reader_depth // (data_width // 8), buffered=True)
            )
            ...
            self.comb += [
                ...
                If(
                    (reader_fifo.level < self.reader_fifo_control.fields.depth[depth_shift:])
                    | (not dynamic_depth),
            ...
            self.sync += If(
                reader_fifo.level < reader_fifo_level_min,
                reader_fifo_level_min.eq(reader_fifo.level),
            )
            # Clear on Status write or when in Instantaneous mode.
            reader_fifo_level_clr = self.reader_fifo_status.re | (
                self.reader_fifo_control.fields.level_mode == 0
            )
            self.sync += If(
                reader_fifo_level_clr, reader_fifo_level_min.eq(2 ** len(reader_fifo_level_min) - 1)
            )
            ...
            self.comb += [
                ...
                If(
                    ...
                    # Min.
                ).Else(self.reader_fifo_status.fields.level[depth_shift:].eq(reader_fifo_level_min))
            ]

        # Writer FIFO.
        if with_writer:
            writer_fifo = ResetInserter()(
                SyncFIFO(dma_layout(data_width), writer_depth // (data_width // 8), buffered=True)
            )
            ...
            self.comb += [
                ...
                If(
                    (writer_fifo.level < self.writer_fifo_control.fields.depth[depth_shift:])
                    | (not dynamic_depth),
            ...
            self.sync += If(
                writer_fifo.level > writer_fifo_level_max,
                writer_fifo_level_max.eq(writer_fifo.level),
            )
            # Clear on Status write or when in Instantaneous mode.
            writer_fifo_level_clr = self.writer_fifo_status.re | (
                self.writer_fifo_control.fields.level_mode == 0
            )
            ...
            self.comb += [
                ...
                If(
                    ...
                    # Min.
                ).Else(self.writer_fifo_status.fields.level[depth_shift:].eq(writer_fifo_level_max))
            ]
    ...
    def __init__(self, endpoint, writer, reader, address_width=32, status_width=32):
        ...
        if status_width == 64:
            ...
            class DMAStatusMSB(LiteXModule):
                def __init__(self, enable, lsb):
                    ...
                    self.sync += [
                        lsb_last.eq(lsb_new),
                        If((lsb_new == 0x0000) & (lsb_last == 0xFFFF), value.eq(value + 1)),
                        If(enable == 0, value.eq(0)),
                    ]
        ...
        fsm.act(
            ...
            If(
                ...
                If(update, NextValue(offset, 0), NextState("WORDS-WRITE")),
            ),
        )
        fsm.act(
            "WORDS-UPDATE",
            NextValue(offset, offset + dwords),
            If(offset == (len(status) - dwords), NextState("IDLE")).Else(NextState("WORDS-DELAY")),
        )
        fsm.act("WORDS-DELAY", NextState("WORDS-WRITE"))
        self.sync += [
            port.source.channel.eq(port.channel),
            port.source.first.eq(1),
            port.source.last.eq(1),
        ...
        fsm.act(
            ...
            port.source.valid.eq(1),
            port.source.we.eq(1),
            If(port.source.ready, NextState("WORDS-UPDATE")),
        )
...
class LitePCIeDMA(LiteXModule):
    ...
    def __init__(
        ...
        writer_buffering_depth=None,
        reader_buffering_depth=None,
        # Monitor.
        with_monitor=False,
        # Status.
        with_status=False,
        ...
        if with_loopback:
            if not (with_writer and with_reader):
                raise ValueError(
                    "Loopback capability requires DMAWriter and DMAReader to be enabled."
                )
        ...
        if with_synchronizer:
            if not (with_writer and with_reader):
                raise ValueError(
                    "Synchronizer capability requires DMAWriter and DMAReader to be enabled."
                )
        ...
        if with_buffering:
            writer_depth = (
                writer_buffering_depth if writer_buffering_depth is not None else buffering_depth
            )
            reader_depth = (
                reader_buffering_depth if reader_buffering_depth is not None else buffering_depth
            )
        ...
        if with_monitor:
            ...
            if with_reader:
                self.reader_monitor = stream.Monitor(
                    self.source, count_width=16, with_underflows=True
                )
        ...
        if with_status:
            if not (with_writer and with_reader):
                raise ValueError(
                    "Status capability requires DMAWriter and DMAReader to be enabled."
                )
    ...
    def add_plugin_module(self, m):
        self.comb += [self.source.connect(m.sink), m.source.connect(self.sink)]
