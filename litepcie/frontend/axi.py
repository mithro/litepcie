...
from litepcie.frontend.dma import LitePCIeDMAReader, LitePCIeDMAWriter, descriptor_layout
...
class LitePCIeAXISlave(LiteXModule):
    def __init__(self, endpoint, data_width=32, id_width=1):
        ...
        # DMA / FIFO / Converter
        self.dma_wr = dma_wr = LitePCIeDMAWriter(endpoint=endpoint, port=port_wr, with_table=False)
        self.fifo_wr = fifo_wr = stream.SyncFIFO(descriptor_layout(), 16)
        self.conv_wr = conv_wr = stream.Converter(
        ...
        self.comb += desc_wr.length.eq(
            (self.axi.aw.len + 1) * (data_width // 8)
        ...
        # DMA / FIFO / Converter
        self.dma_rd = dma_rd = LitePCIeDMAReader(endpoint=endpoint, port=port_rd, with_table=False)
        self.fifo_rd = fifo_rd = stream.SyncFIFO(descriptor_layout(), 16)
        self.conv_rd = conv_rd = stream.Converter(
        ...
        self.comb += desc_rd.length.eq(
            (self.axi.ar.len + 1) * (data_width // 8)
        )  # Transfer length (in bytes)
        ...
        fsm_rd.act(
            ...
            If(
                ...
                If(
                    self.axi.r.last,  # Check if we finished the whole AXI transaction.
                    NextState("READ-IDLE"),
                ),
            ),
        )
