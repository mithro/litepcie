...
class GW5APCIEPHY(LiteXModule):
    ...
    def __init__(self, platform, pads, nlanes=1, data_width=256, cd="sys", bar0_size=0x100000):
        ...
        self._tl_cfg_busdev = CSRStatus(
            13, description="Bus Number and DeviceNumber information for PCIe devices."
        )
        ...
        self._tl_tx_creditsp = CSRStatus(
            32, description="Posted TLP controls the number of credits sent."
        )
        self._tl_tx_creditsnp = CSRStatus(
            32, description="Non-Posted TLP controls the number of credits sent."
        )
        self._tl_tx_creditscpl = CSRStatus(
            32, description="Completion TLP controls the number of credits sent."
        )
        ...
        self.tx_datapath = PHYTXDatapath(
            core_data_width=data_width, pcie_data_width=pcie_data_width, clock_domain=cd
        )
        ...
        for i in range(8):
            self.comb += tl_tx_valid[7 - i].eq(
                Reduce("OR", s_axis_tx.be[i * 4 : (i + 1) * 4]) & s_axis_tx.valid
            )
        ...
        self.ip_params.update(
            ...
            # Control
            o_PCIE_Controller_Top_pcie_linkup_o=self.add_resync(
                self._link_status.fields.status, "sys"
            ),
            o_PCIE_Controller_Top_pcie_ltssm_o=self.add_resync(
                self._link_status.fields.ltssm, "sys"
            ),
            o_PCIE_Controller_Top_pcie_tl_cfg_busdev_o=self.add_resync(
                self._tl_cfg_busdev.status, "sys"
            ),
            ...
            o_PCIE_Controller_Top_pcie_tl_rx_bardec_o=self.add_resync(
                self._tl_rx_bardec.status, "sys"
            ),
            ...
            o_PCIE_Controller_Top_pcie_tl_tx_creditsp_o=self.add_resync(
                self._tl_tx_creditsp.status, "sys"
            ),
            o_PCIE_Controller_Top_pcie_tl_tx_creditsnp_o=self.add_resync(
                self._tl_tx_creditsnp.status, "sys"
            ),
            o_PCIE_Controller_Top_pcie_tl_tx_creditscpl_o=self.add_resync(
                self._tl_tx_creditscpl.status, "sys"
            ),
            # DRP ----------------------------------------------------------------------------------
    ...
    def swap_dwords(self, src, dst):
        ...
        for i in range(ndwords):
            r.append(
                dst[i * 32 : (i + 1) * 32].eq(src[(ndwords - i - 1) * 32 : (ndwords - i - 0) * 32])
            )
        return r
