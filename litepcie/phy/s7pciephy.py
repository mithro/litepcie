...
class S7PCIEPHY(LiteXModule):
    ...
    def __init__(
        ...
        self.specials += Instance(
            "IBUFDS_GTE2", i_CEB=0, i_I=pads.clk_p, i_IB=pads.clk_n, o_O=pcie_refclk
        )
    ...
    def add_sources(self, platform, phy_path, phy_filename=None, user_config=None):
        ...
        if "en_ext_gt_common" not in self.config.keys():
            if platform.device.startswith("xc7a"):
                ...
                platform.toolchain.pre_placement_commands.append(
                    "reset_property LOC [get_cells -hierarchical -filter {{NAME=~pcie_s7/*gtx_common.gtxe2_common_i}}]"
                )
        if self.nlanes != 8:
            platform.toolchain.pre_placement_commands.append(
