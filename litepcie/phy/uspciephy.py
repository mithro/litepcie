...
class USPCIEPHY(LiteXModule):
    ...
    def __init__(
        ...
        self.pcie_phy_params = dict(
            ...
            i_s_axis_rq_tuser=Constant(
                0b0000
            ),  # Discontinue, Streaming-AXIS, EP(Poisioning), TP(TLP-Digest)
    ...
    def add_sources(self, platform, phy_path=None, phy_filename=None):
        if phy_filename is not None:
            ...
            ip_tcl = []
            ip_tcl.append(
                "create_ip -vendor xilinx.com -name pcie3_ultrascale -module_name pcie_us"
            )
        ...
        platform.add_source(
            os.path.join(verilog_path, f"s_axis_rq_adapt_{self.pcie_data_width}b.v")
        )
        platform.add_source(
            os.path.join(verilog_path, f"m_axis_rc_adapt_{self.pcie_data_width}b.v")
        )
        platform.add_source(
            os.path.join(verilog_path, f"m_axis_cq_adapt_{self.pcie_data_width}b.v")
        )
        platform.add_source(
            os.path.join(verilog_path, f"s_axis_cc_adapt_{self.pcie_data_width}b.v")
        )
