...
class C5PCIEPHY(LiteXModule):
    ...
    def __init__(
        ...
        self.specials += Instance(
            "ALT_INBUF_DIFF", i_i=pads.clk_p, i_ibar=pads.clk_n, o_o=pcie_refclk
        )
        ...
        if cd == "pcie":
            ...
            self.comb += [self.sink.connect(tx_buffer.sink), tx_buffer.source.connect(tx_cdc.sink)]
            tx_st = tx_cdc.source
        ...
        self.sync.pcie += [
            If(
                (cfgctl_addr_strobe == 1) & (captured_cfg_addr_reg == 0),
        ...
        self.sync.pcie += [
            If(
                (cfgctl_addr_strobe == 1) & (captured_cfg_addr_reg == 15),
