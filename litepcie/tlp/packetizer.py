...
class LitePCIeTLPHeaderInserter64b3DWs(LiteXModule):
    def __init__(self):
        ...
        self.sync += [
            If(sink.valid & sink.ready, dat.eq(sink.dat), be.eq(sink.be), last.eq(sink.last))
        ]
        ...
        fsm.act(
            ...
            If(
                ...
                If(
                    ...
                    If(count == 1, sink.ready.eq(1), If(~source.last, NextState("DATA"))),
                ),
            ),
        )
        fsm.act(
            ...
            If(last, source.be[4 * 1 : 4 * 2].eq(0x0)).Else(
                source.be[4 * 1 : 4 * 2].eq(sink.be[4 * 0 :])
            ),
            If(
                source.valid & source.ready,
                sink.ready.eq(~last),
                If(source.last, NextState("HEADER")),
            ),
        )
...
class LitePCIeTLPHeaderInserter64b4DWs(LiteXModule):
    def __init__(self):
            ...
            If(
                ...
                If(
                    ...
                    If(
                        count == 1,
                        sink.ready.eq(1),
                        If(~source.last, sink.ready.eq(0), NextState("DATA")),
                    ),
                ),
            ),
        )
        fsm.act(
            ...
            If(source.valid & source.ready, sink.ready.eq(1), If(source.last, NextState("HEADER"))),
        )


class LitePCIeTLPHeaderInserter64b(LitePCIeTLPHeaderInserter3DWs4DWs):
    def __init__(self, fmt):
        LitePCIeTLPHeaderInserter3DWs4DWs.__init__(
            self,
            data_width=64,
...
class LitePCIeTLPHeaderInserter128b3DWs(LiteXModule):
    def __init__(self):
        ...
        fsm.act(
            ...
            If(last, source.be[4 * 3 : 4 * 4].eq(0x0)).Else(
                source.be[4 * 3 : 4 * 4].eq(sink.be[0:])
            ),
            If(
                source.valid & source.ready,
...
class LitePCIeTLPHeaderInserter256b3DWs(LiteXModule):
    def __init__(self):
        ...
        fsm.act(
            ...
            If(last, source.be[4 * 3 : 4 * 8].eq(0x0)).Else(
                source.be[4 * 3 : 4 * 4].eq(sink.be[4 * 0 :]),
                source.be[4 * 4 : 4 * 5].eq(sink.be[4 * 1 :]),
                source.be[4 * 5 : 4 * 6].eq(sink.be[4 * 2 :]),
                source.be[4 * 6 : 4 * 7].eq(sink.be[4 * 3 :]),
...
class LitePCIeTLPHeaderInserter256b4DWs(LiteXModule):
    def __init__(self):
        ...
        fsm.act(
            ...
            If(last, source.be[4 * 4 : 4 * 8].eq(0x0)).Else(
                source.be[4 * 4 : 4 * 5].eq(sink.be[4 * 0 :]),
                source.be[4 * 5 : 4 * 6].eq(sink.be[4 * 1 :]),
                source.be[4 * 6 : 4 * 7].eq(sink.be[4 * 2 :]),
                source.be[4 * 7 : 4 * 8].eq(sink.be[4 * 3 :]),
...
class LitePCIeTLPHeaderInserter512b3DWs(LiteXModule):
    def __init__(self):
        ...
        fsm.act(
            ...
            If(last, source.be[4 * 3 : 4 * 16].eq(0x0)).Else(
                source.be[4 * 3 : 4 * 4].eq(sink.be[4 * 0 :]),
                source.be[4 * 4 : 4 * 5].eq(sink.be[4 * 1 :]),
                source.be[4 * 5 : 4 * 6].eq(sink.be[4 * 2 :]),
                source.be[4 * 6 : 4 * 7].eq(sink.be[4 * 3 :]),
...
class LitePCIeTLPHeaderInserter512b4DWs(LiteXModule):
    def __init__(self):
        ...
        fsm.act(
            ...
            If(last, source.be[4 * 4 : 4 * 16].eq(0x0)).Else(
                source.be[4 * 4 : 4 * 5].eq(sink.be[4 * 0 :]),
                source.be[4 * 5 : 4 * 6].eq(sink.be[4 * 1 :]),
                source.be[4 * 6 : 4 * 7].eq(sink.be[4 * 2 :]),
                source.be[4 * 7 : 4 * 8].eq(sink.be[4 * 3 :]),
...
class LitePCIeTLPPacketizer(LiteXModule):
    def __init__(
        self, data_width, endianness, address_width=32, capabilities=["REQUEST", "COMPLETION"]
    ):
        ...
        if "REQUEST" in capabilities:
            ...
            try:
                force_64b = (
                    LiteXContext.platform.device[:4] in ["xcku", "xcvu", "xczu", "xcau"]
                ) and (data_width in [256, 512])
            except:
                force_64b = False
            ...
            self.comb += [
                ...
                If(req_sink.len > 1, tlp_req.last_be.eq(0xF)).Else(tlp_req.last_be.eq(0x0)),
                tlp_req.first_be.eq(0xF),
                tlp_req.dat.eq(req_sink.dat),
                If(
                    req_sink.we,
                    If(req_sink.len == 1, tlp_req.be.eq(0xF)).Else(
        ...
        if "COMPLETION" in capabilities:
            ...
            self.comb += [
                ...
                If(cmp_sink.last & cmp_sink.first, tlp_cmp.be.eq(0xF)).Else(
        ...
        self.arbitrer = Arbiter(masters=tlp_raws, slave=tlp_raw)
