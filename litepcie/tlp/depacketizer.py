...
class LitePCIeTLPHeaderExtracter64b(LiteXModule):
    def __init__(self):
        ...
        self.sync += If(sink.valid & sink.ready, dat.eq(sink.dat), be.eq(sink.be))
        ...
        fsm.act(
            ...
            If(
                ...
                If(count, If(sink.last, NextValue(last, 1)), NextState("COPY")),
            ),
        )
        fsm.act(
            "COPY",
...
class LitePCIeTLPHeaderExtracter128b(LiteXModule):
    def __init__(self):
        ...
        self.fsm = fsm = FSM(reset_state="IDLE")
        fsm.act(
            "IDLE", NextValue(first, 1), NextValue(last, 0), If(sink.valid, NextState("HEADER"))
        )
        fsm.act(
            "HEADER",
            sink.ready.eq(1),
...
class LitePCIeTLPDepacketizer(LiteXModule):
    def __init__(
        self, data_width, endianness, address_mask=0, capabilities=["REQUEST", "COMPLETION"]
    ):
        # Sink Endpoint.
        ...
        # Dispatch Sources
        self.dispatch_sources = dispatch_sources = {
            "DISCARD": stream.Endpoint(tlp_common_layout(data_width))
        }
        ...
        # Dispatcher
        self.dispatcher = Dispatcher(master=dispatch_sink, slaves=dispatch_sources.values())
        ...
        self.comb += [
            header_extracter.source.connect(
                dispatch_sink, keep={"valid", "ready", "first", "last"}
            ),
        ...
        if "REQUEST" in capabilities:
            self.comb += [
                If(
                    (fmt_type == fmt_type_dict["mem_rd32"])
        ...
        if "COMPLETION" in capabilities:
            self.comb += [
                If(
                    (fmt_type == fmt_type_dict["cpld"]) | (fmt_type == fmt_type_dict["cpl"]),
        ...
        if "CONFIGURATION" in capabilities:
            self.comb += [
                If(
                    (fmt_type == fmt_type_dict["cfg_rd0"]) | (fmt_type == fmt_type_dict["cfg_wr0"]),
            ...
            self.comb += [
                ...
                If(fmt_type == fmt_type_dict["cfg_rd0"], conf_source.we.eq(0)),
                If(fmt_type == fmt_type_dict["cfg_wr0"], conf_source.we.eq(1)),
        ...
        if "PTM" in capabilities:
            self.comb += [
                If(
                    (fmt_type == fmt_type_dict["ptm_req"]) | (fmt_type == fmt_type_dict["ptm_res"]),
