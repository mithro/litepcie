    ...
    def __init__(self, data_width, address_width, max_pending_requests, cmp_bufs_buffered=True):
        ...
        req_fsm.act(
            ...
            If(
                ...
                ).Elif(tag_queue.source.valid & req_queue.sink.ready, NextState("SEND-READ-REQ")),
            ),
        )
        req_fsm.act(
            "SEND-WRITE-REQ",
            ...
            If(req_source.valid & req_source.ready & req_source.last, NextState("WAIT-REQ")),
        )
        self.comb += [
        ...
        req_fsm.act(
            ...
            # End Request and return to Wait on last valid cycle.
            If(
                req_source.valid & req_source.ready & req_source.last,
        ...
        for i in range(max_pending_requests):
            ...
            cmp_buf = SyncFIFO(
                completion_layout(data_width), cmp_buf_depth, buffered=cmp_bufs_buffered
            )
        ...
        cmp_fsm.act(
            ...
            If(fill_tag == (max_pending_requests - 1), NextState("WAIT")),
        )
        cmp_fsm.act(
            "WAIT",
            # Wait for a TLP Completion...
            If(cmp_sink.valid & cmp_sink.first, NextState("RUN")).Else(cmp_sink.ready.eq(1)),
        )
        cmp_fsm.act(
            "RUN",
            # Connect Control-Path.
            ...
            If(
                cmp_sink.valid & cmp_sink.ready & cmp_sink.last,
                If(cmp_sink.end, tag_queue.sink.valid.eq(1), tag_queue.sink.tag.eq(cmp_sink.tag)),
                NextState("WAIT"),
            ),
        )
