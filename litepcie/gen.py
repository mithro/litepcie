...
from litepcie.frontend.ptm import PCIePTMSniffer, PTMCapabilities, PTMRequester
...
def get_axi_dma_ios(_id, data_width, with_writer=True, with_reader=True):
    ...
    ios += [(f"dma{_id}_status", 0, *enables)]

    # DMA Writer AXI.
    if with_writer:
        ios += [
            (
                f"dma{_id}_writer_axi",
                0,
...
class LitePCIeCore(SoCMini):
    ...
    def __init__(self, platform, core_config):
        ...
        class DMAParams:
            def __init__(
                self, writer, reader, buffering, loopback, synchronizer, monitor, data_width
            ):
                self.writer = writer
                self.reader = reader
        ...
        if core_config.get("msi_x", False):
            ...
            self.pcie_msi = LitePCIeMSIX(
                self.pcie_endpoint, width=64, default_enable=msi_x_default_enable
            )
            self.comb += self.pcie_msi.irqs[32 : 32 + core_config["msi_irqs"]].eq(
        ...
        if core_config.get("ptm", False):
            ...
            for _from, _to in pcie_ptm_sniffer_connections:
                platform.toolchain.pre_optimize_commands.append(
                    f"set pin_driver [get_nets -of [get_pins {_to}]]"
                )
                platform.toolchain.pre_optimize_commands.append(
                    f"disconnect_net -net $pin_driver -objects {_to}"
                )
                platform.toolchain.pre_optimize_commands.append(
                    f"connect_net -hier -net {_from} -objects {_to}"
                )
    ...
    def generate_documentation(self, build_name, **kwargs):
        ...
        os.system("sphinx-build -M html documentation/ documentation/_build".format())
