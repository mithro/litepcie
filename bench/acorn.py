...
class LitePCIeSoC(SoCMini):
    def __init__(self, platform, sys_clk_freq=int(125e6)):
        ...
        # SoCMini ----------------------------------------------------------------------------------
        SoCMini.__init__(
            self, platform, sys_clk_freq, ident="LitePCIe standalone example design on Acorn"
        )
        ...
        platform.toolchain.pre_placement_commands.append(
            "set_false_path -from [get_clocks userclk2] -to [get_clocks litepciesoc_clkout]"
        )
        platform.toolchain.pre_placement_commands.append(
            "set_false_path -from [get_clocks litepciesoc_clkout] -to [get_clocks userclk2]"
        )
