...
def main():
    ...
    parser.add_argument(
        "--family", default="7series", help="FPGA family.", choices=["7series", "ultrascale"]
    )
    ...
    ltssm_dict = {"7series": PCIE_LTSSM_7SERIES, "ultrascale": PCIE_LTSSM_ULTRASCALE}[args.family]
    ...
    while True:
        ...
        if not valid:
            break
        print(
            f"[0x{ltssm_old:02x}] {ltssm_dict.get(ltssm_old, 'reserved'):<32s} -> [0x{ltssm_new:02x}] {ltssm_dict.get(ltssm_new, 'reserved'):<32s}{('[Overflow, possible unknown intermediate states]' if overflow else ''):s}"
        )
