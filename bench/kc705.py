...
class LitePCIeSoC(SoCMini):
    ...
    def __init__(self, platform, speed="gen2", nlanes=4):
        ...
        SoCMini.__init__(
            ...
            platform,
            sys_clk_freq,
            ident=f"LitePCIe example design on KC705 ({speed}:x{nlanes})",
        )
        ...
        self.pcie_phy = S7PCIEPHY(
            platform,
            platform.request(f"pcie_x{nlanes}"),
        ...
        self.pcie_endpoint = LitePCIeEndpoint(
            self.pcie_phy, endianness="big", max_pending_requests=8
        )
