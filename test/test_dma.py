        ...
        while True:
            ...
            if (yield self.sink.valid):
                ...
                if irq_vector & DMA_READER_IRQ:
                    ...
                    if self.debug:
                        print(f"[MSI] dma_reader_irq (n: {self.dma_reader_irq_count:d})")
                    irq_clear |= DMA_READER_IRQ

                if irq_vector & DMA_WRITER_IRQ:
                    self.dma_writer_irq_count += 1
                    if self.debug:
                        print(f"[MSI] dma_writer_irq (n: {self.dma_writer_irq_count:d})")
    ...
    def dma_test(self, data_width, address_width, test_size=1024):
        ...
        def main_generator(dut, nreads=8, nwrites=8):
            ...
            for i in range(nwrites):
                yield from dma_writer_driver.program_descriptor(
                    test_size + (test_size // 8) * i, test_size // 8
                )
        ...
        class DUT(LiteXModule):
            def __init__(self, data_width, address_width):
                ...
                self.endpoint = LitePCIeEndpoint(
                    self.host.phy, address_width=address_width, max_pending_requests=8
                )
                ...
                self.dma_reader = LitePCIeDMAReader(
                    self.endpoint, dma_reader_port, address_width=address_width
                )
                self.dma_writer = LitePCIeDMAWriter(
                    self.endpoint, dma_writer_port, address_width=address_width
                )
